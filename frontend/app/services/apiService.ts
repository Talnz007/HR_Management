...
class ApiService {
  ...
  async getDepartments() {
    return (await this.api.get("/departments/")).data
  }
  async uploadProfilePicture(userId: string, file: File) {
    const formData = new FormData()
    formData.append('file', file)
    return (await this.api.post(`/users/${userId}/profile-picture`, formData)).data
  }
  async deleteProfilePicture(userId: string) {
    return (await this.api.delete(`/users/${userId}/profile-picture`)).data
  }
  async getMyProfile() {
    return (await this.api.get("/users/me/profile")).data
  }
