...
A web-based human resource management system with role-based access control, attendance tracking, leave approvals, and payroll processing.
...
### Admin Dashboard
Workforce summary showing active headcount, today's attendance, pending leave requests, and monthly payroll totals.

![Admin Dashboard](docs/screenshots/admin_dashboard.png)

### Authentication
Login portal with role routing (Admin, Manager, Employee) and password reset support.
...
- **Role-Based Access Control (RBAC):** Distinct permissions and route guards for Admin, Manager, and Employee roles.
- **Employee Directory:** Add, update, and manage employee profiles, departments, job titles, and contact information.
- **Attendance Logging:** Daily clock-in/clock-out tracking with timestamps and attendance history.
- **Leave Management:** Employees submit leave requests; managers and admins review, approve, or reject them.
- **Payroll Processing & Payslips:** Compute base salaries, bonuses, and deductions, then generate client-side PDF payslips using jsPDF.
- **Real-Time Events:** Socket.IO integration pushes instant status updates across client sessions.
- **Metrics Telemetry:** Built-in Prometheus instrumentation at `/metrics` tracking request duration, response codes, and throughput.
...
- **Database & Storage:** PostgreSQL, SQLAlchemy 2.0, Alembic, Redis
...
To view real-time latency and request rate counters, check Prometheus:
```bash
